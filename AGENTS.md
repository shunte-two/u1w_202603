...
## 2026-03-28 会話スキップ案内メモ

- PlayerPrefs の chapter1 到達フラグが ON の場合、Game シーン開始時に画面左上へ「スペースキー長押しで会話スキップ」を 2 秒表示し、その後フェードアウトする。
- この案内表示は Game シーン内の専用 UI コンポーネントで管理し、表示条件判定は既存のチャプター到達保存 API を参照する。- ConversationSkipHintOverlay はコンポーネント本体を非活性化せず、CanvasGroup の alpha のみで初期非表示とフェード制御を行い、Start 未実行による再生漏れを防ぐ。